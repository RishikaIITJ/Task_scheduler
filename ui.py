...
def run_cpp(commands):
    if not binary_exists():
        return "[ERROR] Binary not found. Compile with: g++ -o scheduler main.cpp"
    stdin_text = "\n".join(commands + ["exit"]) + "\n"
    try:
...
# ── Parser: keep only FCFS + Priority ────────────────────────────────────────
...
def fcfs_order(tasks):
    """Return tasks sorted by start time (FCFS order)."""
    return sorted(tasks, key=lambda t: t["start_time"])

def priority_order(tasks):
    """Return tasks sorted by priority then start time."""
...
with st.sidebar:
    ...
    if binary_exists():
        ...
        st.markdown("<div class='status-err'>binary not found - compile first</div>",
                    unsafe_allow_html=True)
        st.code("g++ -o scheduler main.cpp")
